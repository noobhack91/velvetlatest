...
class ModelsController extends Controller
{
    ...
    public function models(){
        ...
        $users = User::where('role','=','model')->paginate(10);
        return view('admin.models.models')->with(["users"=>$users]);
    ...
    public function addModel(){
		...
		return view('admin.models.signupNew')->with(['countries'=>$countries,'pages'=>$pages]);
    ...
    public function store(Request $request){
       ...
       /* Mail::send('emails.emailVerificationEmail', ['token' => $token], function($message) use($request){
        ...
        if(!is_null($user))
        {
            ...
            $mail=\Mail::to($to)->send(new \App\Mail\MyTestMail($details));
            return redirect(route('models'))->with('message', 'Model Inserted Please check email for password');
        }*/
    }
    ...
    public function edit($id){
        ...
        if(isset($userprofiles))
        {
            ...
            $country = Country::where('name', '=', $countryName)->first();
             ...
             $pages = Page::orderBy('page_name')->get();
             return view ('admin.models.completeprofile')->with(["user"=>$user,'modeldata' => $modeldata,'gallarydata' => $gallarydata,'privatethings' => $privatethings,'states' => $states, 'pages'=>$pages]);
     ...
     public function update(Request $request,$id){
        ...
        if($usermodel)
        {
            ...
            return redirect(route('models'))->with('message', 'Model Data Has Been Updated');
        }
        else
        {
            $modelmeta = new Modeles;
    ...
    public function GallayDelete($id)
    {
        ...
        $gallary->delete();
        return redirect(route('models'))->with('message', 'Gallay Updated');
    }
    ...
    public function delete($id)
    {
        ...
        return redirect(route('models'))->with('message', 'Model Has Been deleted');

    }

    public function modelverification()
    {

        $users = User::where ('role','=','model')->where ('image_verification_status','=','0')->paginate(10);
        return view('admin.models.modelverification')->with(["users"=>$users]);

    }

    public function verificationedit($id)
    {

        $id = decrypt($id);
        $models = Modeles::where('user_id',$id)->first();
        $users = User::where('id',$id)->first();
        return view('admin.models.completeverification')->with(["models"=>$models,"users"=>$users]);
    ...
    public function verificationupdate(Request $request, $id)
    {
        $id = decrypt($id);
        $users = User::where('id',$id)->first();
        if( ($users) && ($request->submit) )
        {
        User::where('id',$id)->update(['image_verification_status' => 1]);
        }
        return redirect(Route('admin.modelVerification'));
    }
